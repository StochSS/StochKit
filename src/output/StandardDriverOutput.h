/**
  ...
  @brief standard class used in SerialIntervalSimulationDriver to handle output
...
//		std::cout << "in unserialize, inputBool is "<<inputBool<<"\n";
 ...
 public:
  ...
  double calibratorStat;
#ifdef USER_OUTPUT
  UserOutput<_populationVectorType> user_output;
#endif
 ...
 public:
...
#ifdef USER_OUTPUT 
  StandardDriverOutput(bool keepStats=true,bool keepTrajectories=true, bool keepHistograms=true, bool keepUserOutput=false):trajectories(),stats(),keepStats(keepStats),keepTrajectories(keepTrajectories), keepHistograms(keepHistograms), keepUserOutput(keepUserOutput)
  {}
#else
...
#ifdef USER_OUTPUT
  void setKeepUserOutput(bool keep) {
    keepUserOutput=keep;
  }
#endif
  bool getKeepStats(){
  ...
  bool getKeepHistograms(){
      return keepHistograms;
  }
  bool getKeepTrajectories(){
      return keepTrajectories;
  }
#ifdef USER_OUTPUT
  bool getKeepUserOutput() {
...
#ifdef USER_OUTPUT
    user_output.setOutputTimes(outputTimes);
#endif
  }
  ...
  /**
    @brief gets called by the ensemble_simulate function after SerialIntervalSimulationDriver passes the
    output object to the ensemble_simulate function

    @see SerialIntervalSimulationDriver
    @see ensemble_simulate
  */
...
#ifdef USER_OUTPUT
    bool userOutputInitialized=true;
#endif
    //initialize stats
    if (keepStats) {
...
#ifdef USER_OUTPUT
    //initialize user_output data
    if (keepUserOutput) {
      userOutputInitialized=user_output.initialize(realizations, startTime, endTime, samplePopulationVector);
    }
#endif

#ifdef USER_OUTPUT
    if (statsInitialized && trajectoriesInitialized && histogramsInitialized && userOutputInitialized) {
      return true;
    }
#else
...
#ifdef USER_OUTPUT
    if (keepUserOutput) {
      user_output.record(realization,interval,population);
    }
#endif
  }

  void recordCalibratorStat(double calibratorStat)
  {
    this->calibratorStat = calibratorStat;
  }
  ...
  /**
    -other write functions simply call equivalent method in stats member of StandardDriverOutput
    class, but calibrator stat is simple enough that we can just write it here
  */
  void writeCalibratorStatToFile(std::string filename)
  {
    std::ofstream outfile;
    ...
    if (!outfile) {
      std::cerr << "StochKit ERROR (StandarDriverOutput::writeCalibratorStatToFile): Unable to open output file\n";
      exitFunc(1);
    }

    try{
        outfile << calibratorStat;
        outfile.close();
    }
    catch (...) {
      std::cout << "StochKit ERROR (StandarDriverOutput::writeCalibratorStatToFile): error writing data to output file.\n";
      exitFunc(1);
    }
