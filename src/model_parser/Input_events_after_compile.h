...
namespace STOCHKIT
{
	/**
		...
		@brief parses XML file with the help of Input_events (as a base class) then passes this info
		using the writeEvents function
		@see Input_Events
	*/
 ...
 public:
 	/**
		@brief creates propensitiesFunctor for events solvers

		-usually used to pass in to a solver constructor when create a solver
		-usally the propenstitesFunctor returned is of the type CustomPropensitySet

		@see CustomPropensitySet.h
 	*/
	_propensitiesFunctorType writePropensities()
	{
		_propensitiesFunctorType propensitiesList(this->ParametersList);
	...
	/**
		@brief creates an ready-to-go eventHandler after XML file has been parsed

		-creates new StateBasedTriggerEvent and TimeBasedTriggerEvent objects and inserts
		the appropriate actions that these events cause into those objects. These actions
		are stored as a vector of EventAction objects
		-EventActions are functors and take in a reference to solver so they can change
		the parameters and states of solvers

		@see TimeBasedTriggerEvent
		@see StateBasedTriggerEvent
		@see EventAction
	*/
