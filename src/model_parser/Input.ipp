...
linkParametersAndReactions()
{
	...
	ParametersList.finishLinkingWithReactions();	
	return true;
}
...
writeStoichiometry()
{
		...
		// initialize values to 0 
 		VectorUtility::initializeVectorToZero(nu);
	#endif
...
writeReactants()
{
	...
	std::vector<std::vector<int> > reactantList(NumberOfReactions, temp);

	//calculate reactants and write
	class SpeciesReference *cur_species = NULL;
	...
	for(int i=0; i<NumberOfReactions; ++i){ // the i-th reaction
		...
		for(unsigned int j=0; j<ReactionsList[i].Reactants.size(); ++j){
			cur_species = &ReactionsList[i].Reactants[j];
			reactantList[i][j]=cur_species->Index;
			if(cur_species->Stoichiometry==-2)
				reactantList[i][1]=cur_species->Index;
		}
	}
	return reactantList;
}
...
writeRates()
{
	std::vector<double> rateList(NumberOfReactions);
	std::string rate;
	...
	for(int i=0; i<NumberOfReactions; ++i){ // the i-th reaction
		...
		for(int j=0; j<NumberOfParameters; j++)
		{
			if(ReactionsList[i].Rate==ParametersList[j].Id)
			{
				rateList[i]=ParametersList[j].Value;
				break;
			}
		}
	}
