...
namespace STOCHKIT
{
	...
	protected:
		...
		//! \brief Operations need to be done before the simulation
		void prepare();

		//! \brief Initialize values before each realization
		void initialize(double startTime);

		/* \brief Select the stepsize for each step
			\param noncriticalStepsize returns the stepsize from non critical reactions
			\param criticalStepsize returns the stepsize from the critical reactions
		*/
		void selectTau(double &noncriticalStepsize, double &criticalStepsize);
		
		/* \brief Sample the number of firings in each reaction channel in one step

			If critical reaction fires, it returns the index of the fired critical reaction, otherwise it returns -1
			\param leapSize the stepsize
			\param runCritical true: a critical reaction fires; false: no critical reaction fires
		*/
		int selectReactions(double leapSize, bool runCritical);

		/* \brief Fire the sampled reactions

			If negative population occurs, it rolls back the step and returns false. Otherwise it returns true
			\param criticalIndex the index of the fired critical reaction. If no critical reaction fires, set it to -1
		*/
		bool fireReactions(int criticalIndex);

		//methods for critical reactions
		/* \brief Fire the critical reaction

			If the argument reactionIndex equals -1, it returns false. Otherwise it returns true
			\param reactionIndex the index of the fired critical reaction.
		*/
		bool critical_fireReaction(int reactionIndex);

		/* \brief Roll back the critical reaction

			If negative population occurs, it roll back the fired critical reaction in this step
			\param reactionIndex the index of the fired critical reaction.
		*/
		void critical_rollBack(int reactionIndex);

		//	delete pure product
		std::list<std::size_t> trimed_list;
