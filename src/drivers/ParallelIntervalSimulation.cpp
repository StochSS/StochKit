...
//static const initialization
	const size_t ParallelIntervalSimulation::nSolvers = 3; //number of solvers calibrator is comparing
	const std::string ParallelIntervalSimulation::solverNames[3] = {"ODM", "ConstantTime", "NRM"};
//end static const initialization
	...
	ParallelIntervalSimulation::ParallelIntervalSimulation(int ac, char* av[]):commandLine(ac,av), masterProc(0), engine((boost::uint32_t) std::time(0))
	{
		...
		if (numberOfProcesses==0) {
			...
			if (numberOfProcesses==0) {
				std::cout << "StochKit MESSAGE (ParallelIntervalSimulation()): unable to detect number of processors.  Simulation will run on one processor." << std::endl;
				numberOfProcesses=1;
			}
		}
		...
		for (std::size_t i=0; i<numberOfProcesses; i++) {
			...
			if (subRealizations>0) {
				...
				if (commandLine.getKeepUserOutput()) {
					command+="--user-output-dir user_output/.parallel --user-output-file user_output_thread_"+threadNumString+".dat";
				}
#endif
	...
	void ParallelIntervalSimulation::runCalibrator(std::string executableName) {
		boost::uniform_int<> distribution(1, RAND_MAX) ;
		boost::variate_generator<boost::mt19937, boost::uniform_int<> > seedOfNewThread(engine, distribution);
		...
		//need 1 processor for master and 3 for each of the 3 different methods
		//this way the master processor doesn't endure timing conflicts from having to
		//organize other threads as well as simulate
		switch( numberOfProcesses >= (nSolvers+1) )
		{
			//run each of the methods in parallel
			case 1:
       		{
				for(std::size_t i=1; i<=nSolvers; i++)
				{
					...
					newseed = seedOfNewThread();
					seedString=StandardDriverUtilities::size_t2string(newseed);
					command=executableName+ " -r 1"+commandLine.getCmdArgs()+" --method "+ solverNames[i-1] + " --serial --use-existing-output-dirs ";
					...
					command+=" --calibrator-dir calibrator_output --calibrator-file calibrator_thread_"+threadNumString+".dat";
       		...
       		//run each of the methods serially only difference is that the join_all() is in the for loop
       		//causing the thread to finish before another thread can start
     		default:
	    	{
	...
	inline void ParallelIntervalSimulation::executable(std::string command) {
		cmdExec(command.c_str());
	}

	std::size_t ParallelIntervalSimulation::assignment(std::size_t totalRealizations, std::size_t threadid) {
	...
	void ParallelIntervalSimulation::writeCommandLogAndThreadLog()
	{
		std::string threadNumString="";
		std::string command="";
		...
		for (std::size_t i=0; i<numberOfProcesses; i++) {
			...
			int emptyLog=cmdExec(command.c_str());
#endif
...
#endif
			int grepERROR=cmdExec(command.c_str());
			if (grepERROR==0) {
...
#else
				std::cout << "error messages written to log file: \"" << commandLine.getOutputDir()<<"/log.txt\"" << std::endl;
#endif
				exitFunc(1);//exit here since merging stats probably won't work and any other data could be invalid
			}
			else {
#ifdef WIN32
				std::cout << "StochKit WARNING: messages written to log file: \"" << OutputDir<<"\\log.txt\"\n";   
#else
				std::cout << "StochKit WARNING: messages written to log file: \"" << commandLine.getOutputDir()<<"/log.txt\"" << std::endl; 
#endif
			}
		}
	}
...
#ifdef USER_OUTPUT
		//merge user-output here
		if (commandLine.getKeepUserOutput()) {
			std::cout << "creating user output files...\n";
			std::cout.flush();

			typedef UserOutput<StandardDriverTypes::populationType> UserOutputType;

			//masterProc is first processor that was assigned at least one realization
			//we do assume that at least one realization was run
#ifdef WIN32
			UserOutputType masterUserOutput=UserOutputType::createFromFiles(OutputDir+"\\"+commandLine.getUserOutputDir()+"\\.parallel\\user_output_thread_"+StandardDriverUtilities::size_t2string(masterProc)+".dat");
#else
			UserOutputType masterUserOutput=UserOutputType::createFromFiles(commandLine.getOutputDir()+"/"+commandLine.getUserOutputDir()+"/.parallel/user_output_thread_"+StandardDriverUtilities::size_t2string(masterProc)+".dat");
#endif

			UserOutputType tmpUserOutput;
			...
			for (std::size_t i=masterProc+1; i<numberOfProcesses; ++i) {
				if (processorRealizationAssignments[i]>0) {
					threadNumString=StandardDriverUtilities::size_t2string(i);
#ifdef WIN32
					tmpUserOutput=UserOutputType::createFromFiles(OutputDir+"\\"+commandLine.getUserOutputDir()+"\\.parallel\\user_output_thread_"+threadNumString+".dat");
#else
					tmpUserOutput=UserOutputType::createFromFiles(commandLine.getOutputDir()+"/"+commandLine.getUserOutputDir()+"/.parallel/user_output_thread_"+threadNumString+".dat");
#endif
					masterUserOutput.merge(tmpUserOutput);
				}
			}

#ifdef WIN32
			masterUserOutput.writeDataToFile(OutputDir+"\\"+commandLine.getUserOutputDir()+"\\user_output.dat");
#else
			masterUserOutput.writeDataToFile(commandLine.getOutputDir()+"/"+commandLine.getUserOutputDir()+"/user_output.dat");
#endif

			//after merging user output, remove user output parallel directory
#ifdef WIN32
			boost::filesystem::remove_all(OutputDir+"\\"+commandLine.getUserOutputDir()+"\\.parallel");
#else
			boost::filesystem::remove_all(commandLine.getOutputDir()+"/"+commandLine.getUserOutputDir()+"/.parallel");
#endif
		}
#endif
	...
	void ParallelIntervalSimulation::mergeCalibratorOutput() {
		writeCommandLogAndThreadLog();
		
		//retrieve times from files
		std::vector<double> timingResults(nSolvers);
		for(std::size_t i=1; i<=nSolvers; i++)
		{
			std::string threadNumString=StandardDriverUtilities::size_t2string(i);

			#ifdef WIN32
			std::string outputFileName=commandLine.getOutputDir()+"\\"+commandLine.getCalibratorDir()+"\\calibrator_thread_"+threadNumString+".dat";
			#else
			std::string outputFileName=commandLine.getOutputDir()+"/"+commandLine.getCalibratorDir()+"/"+"calibrator_thread_"+threadNumString+".dat";
			#endif

			std::ifstream fin(outputFileName.c_str());
	    	double calibratorTime;
	    	fin >> calibratorTime;
	    	
	    	timingResults[i-1] = calibratorTime;
		}

		//display times
		std::vector<double> timingResultsSorted = timingResults;
		std::sort( timingResultsSorted.begin(), timingResultsSorted.end() );

		std::cout << std::endl;
		std::cout << "SOLVER" << "\t AVERAGE FIRING TIME PER REACTION" << std::endl;

		for(size_t i=1; i<=nSolvers; i++)
		{
			std::string solverName;
			for(size_t j=0; j<nSolvers; j++)
			{
				if( timingResultsSorted[i-1] == timingResults[j] )
					solverName = solverNames[j];
			}

			if(i == 1)
				std::cout << i << "-" << solverName << ":\t" << timingResultsSorted[i-1] << " <--RECOMMENDEND SOLVER" << std::endl;
			else
				std::cout << i << "-" << solverName << ":\t" << timingResultsSorted[i-1] << std::endl;
		}
	}
	...
	std::string ParallelIntervalSimulation::modifyCmdArgsSeed(std::string commandLineArguments, std::string newSeed) {
		...
		std::size_t cutStart = 0; //dummy values
		std::size_t cutEnd = 0; //dummy values
...
#ifdef USER_OUTPUT
		if (commandLine.getKeepUserOutput()) {
			std::cout << "StochKit WARNING: user output enabled, simulation will generate unknown amounts of data (at least "<<dataSize/1000000<<"MB in at least "<< numFiles <<" files) ...initializing...\n";
		} else if (numFiles>10000 || dataSize>10000000) {
...
#else
		if (commandLine.getKeepUserOutput()) {
			std::cout << "StochKit WARNING: user output enabled but not compiled yet, simulation will not generate user output...initializing...\n";
		}
