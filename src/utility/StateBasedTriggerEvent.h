...
namespace STOCHKIT
{
  /**
    ...
    @brief class for state events

    -created in Input_events_after_compile in function writeEvents()
    -contains a triggerFunctor (CustomStateBasedTrigger) for deciding when to fire based on the current population

    @see Input_events_after_compile
    @see CustomStateBasedTrigger
  */
