...
/*#ifdef WIN32
			...
			if (commandLine.getKeepUserOutput()) {
				boost::filesystem::create_directory(outputDir+"\\"+commandLine.getUserOutputDir());
				if (parallel) {
					boost::filesystem::create_directory(outputDir+"\\"+commandLine.getUserOutputDir()+"\\.parallel");
				}
			}
			...
			if ( commandLine.shouldCalibrate() ) {
				boost::filesystem::create_directory(outputDir+"/"+commandLine.getCalibratorDir());
				if (parallel) {
					boost::filesystem::create_directory(outputDir+"/"+commandLine.getCalibratorDir()+"/.parallel");
				}
			}
...
//#endif
			...
			std::cerr << "StochKit ERROR (StandardDriverUtilities::createOutputDirs): error creating output directory." << std::endl;
			exitFunc(1);
		}
	}
	...
	void StandardDriverUtilities::compileMixed(std::string executableName, const CommandLineInterface& commandLine, const boost::filesystem::path current_path, bool events) {
		...
		if (commandLine.getRecompile()) {
			try {
				...
				std::cerr << "StochKit ERROR (StandardDriverUtilities::compileMixed): error creating output directory." << std::endl;
				exitFunc(1);
			}
			...
			char *const modelFileName = static_cast<char *>(_alloca(tempname.length() + 2048) );
			char *const tempname_c_str = static_cast<char *>(_alloca(generatedCodeDir.length() + 2048) );
			memset(modelFileName, 0, tempname.length() + 2048);
			memset(tempname_c_str, 0, generatedCodeDir.length() + 2048);
...
if(events){
			...
			std::cout << "StochKit MESSAGE: compiling generated code...this will take a few moments..." << std::endl;
			int returnValue=cmdExec(makeCommand.c_str());
			...
			/*	std::string command="copy /B "+commandLine.getGeneratedCodeDir()+"\\.compile-log.txt "+commandLine.getGeneratedCodeDir()+"\\compile-log.txt";
				...
				std::cout << "Check log file \"" << generatedCodeDir<<"\\compile-log.txt\" for error messages." << std::endl;
				exitFunc(1);
			}
...
#else
	void StandardDriverUtilities::compileMixed(std::string executableName, const CommandLineInterface& commandLine, bool stochkit_system_app, bool events) {
		if (commandLine.getRecompile()) {
			try {
			...
			std::string tempname;
			tempname=commandLine.getModelFileName();
			char modelFileName[tempname.length() + 2048];
			char tempname_c_str[commandLine.getGeneratedCodeDir().length() + 2048];
			memset(modelFileName, 0, tempname.length() + 2048);
			memset(tempname_c_str, 0, commandLine.getGeneratedCodeDir().length() + 2048);
			strcpy(modelFileName,tempname.c_str());
...
//#endif
			...
			std::string makefilePath=commandLine.getCommandDir()+"/../";

			//updated so generated code path is a full path sanft 2011/03/21
			std::string makeCommand="";
			if( stochkit_system_app == true ){
//				makeCommand+=(std::string)"cd $STOCHKIT_HOME/src; make "+executableName+" GENERATED_CODE_PATH="+commandLine.getGeneratedCodeDir()+" CURRENT_PATH="+currentPath+"--silent";
				makeCommand+=(std::string)"cd \"" + stochkit_home_string + "/src\"; make "+executableName+" GENERATED_CODE_PATH=\""+commandLine.getGeneratedCodeDir()+"\" STOCHKIT_HOME=\"" +stochkit_home_string +"\" --silent";
			} else {
				makeCommand+=(std::string)"cd \"" +makefilePath+ "\"; make "+executableName+" GENERATED_CODE_PATH=\""+commandLine.getGeneratedCodeDir()+"\" CURRENT_PATH=\""+currentPath+"\" MAKEFILE_PATH=\"" +makefilePath+ "\"--silent";
			}
			...
			if (returnValue!=0) {
				...
				std::cout << "Check log file \"" << commandLine.getGeneratedCodeDir()<<"/compile-log.txt\" for error messages." << std::endl;
				exitFunc(1);
			}
