...
namespace STOCHKIT
{
 ...
 public:
  ...
  typedef boost::function<bool (double, _populationVectorType&, ParameterSet&)> triggerType;

	CustomStateBasedTrigger(triggerType func, ParameterSet& solverParametersList, bool allow): // by default, don't allow multiple firings
	  disabled(false),
	  internalState(false),
